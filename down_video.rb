...
# The module get all the wanted informations from webpage and store in yml file 
module Soku
  class Video
  ...
  def self.search(keyword)
    ...
    open(URI::encode(search_url)) do |result|
      ...
      doc.css('ul[class=v]').each_with_index do |ul, index|
        video = Video.new   #css always returns a array so need [0] below
        video.title       = ul.css('li.v_title').css('a')[0]['title'].strip
    ...
    # Dump the information from videos to yml file
...
module DownloadableUrl
  # TO Do: The method was defined to generate the log file
  def self_generate(content)
      Logger.new('log.yml').info content
  end
end
  class Regexp
  # scan the yml file generated by SOKU module and get the final url
  def url_scan(yml_file)
    Enumerator.new do |find|
      yml_file.scan(self) do
        find << Regexp.last_match
      end
    end
  end
end

# Not sure how to apply indentation for below codes
Soku::search("ruby")
  video_yml = YAML::load(File.open("#{VIDEOS}"))
    start_indexs = /http:\/\/f/.url_scan(video_yml.to_s).map { |m| m.offset(0)[0] }
